...
def get_top_n_genres(artists: List[Dict[str, Union[str, int, List[str]]]], n: int = 5):
    ...
    genres_sorted = sorted(
        list(set(genres)), key=lambda g: genres_counted[g], reverse=True
    )
...
def get_top_n_artists(artists: List[Dict[str, Union[str, int, List[str]]]], n: int = 5):
...
def get_top_n_tracks(tracks: List[Dict[str, Union[str, int, np.ndarray]]], n: int = 5):
...
def plot_top_track_popularity_distribution(
    tracks: List[Dict[str, Union[str, int, np.ndarray]]],
    username: str,
    out_dir: str = ".",
) -> None:
    ...
    fig, ax = plt.subplots()
    sns.histplot(
        data={"popularity": [track["popularity"] for track in tracks]},
        x="popularity",
        ax=ax,
    )
...
def plot_track_features(
    ...
    sns.barplot(data=features_df, x="feature", y="value")
...
def get_most_similar_track(
    user_1_tracks: List[Dict[str, Union[str, int, Dict[str, float]]]],
    ...
    user_1_tracks_unique = [
        track
        for track in user_1_tracks
        if track["uri"] not in [track["uri"] for track in user_2_tracks]
    ]

    user_2_tracks_unique = [
        track
        for track in user_2_tracks
        if track["uri"] not in [track["uri"] for track in user_1_tracks]
    ]
    ...
    user_1_vectors = np.vstack(
        [track["audio_features"] for track in user_1_tracks_unique]
    )

    user_2_vectors = np.vstack(
        [track["audio_features"] for track in user_2_tracks_unique]
    )

    # get average distance for each track from user_2 to tracks from user_1
    distances = np.array(
        [np.average(np.linalg.norm(v - user_1_vectors, axis=1)) for v in user_2_vectors]
    )
...
def plot_user_track_pca(
    ...
    user_1_df = pd.DataFrame(
        [track["audio_features"] for track in user_1_tracks], columns=AUDIO_FEATURES
    )
    user_1_df["user"] = user_1_name

    user_2_df = pd.DataFrame(
        [track["audio_features"] for track in user_2_tracks], columns=AUDIO_FEATURES
    )
    ...
    pca_df = pd.DataFrame(data=pca_audio_features, columns=["pc1", "pc2"]).assign(
